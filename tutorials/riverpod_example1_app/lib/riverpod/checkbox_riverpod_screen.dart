...
import 'package:riverpod_example1_app/riverpod/drinks_notifier.dart';
import 'package:riverpod_example1_app/widgets/drinks_widget.dart';

import '../constants.dart';

final drinksProvider = StateNotifierProvider(
  (ref) => DrinkStateNotifier(
    [Drink("Water", false), Drink("Cuba Libre", false), Drink("Pina Colada", false), Drink("Havana Cola", false)],
  ),
);

final selectedProvider = Provider<List<Drink>>(
  (ref) {
    return ref.watch(drinksProvider.state).where((element) => element.selected).toList();
  },
);
